...
- The owner asked mid-review for one structural change: the customer account is an organization, not a tenant. That rename is applied throughout and listed under change 11.
...
**11. Tenant renamed to organization (owner request).** `tenant_id` is `org_id` everywhere in SQL, the schema spec, OpenAPI, the API reference, the repo schemas, and the samples. Tables live in the product's schema layout: `org.organizations`, `org.org_users` as the junction to `auth.users`, `workspace.workspaces`, and the ARP control tables in `oxagen`. `oxagen.tenant_id` is `oxagen.org_id`; API paths are `/organizations/{org_id}`. Prose that still says tenant, company, or customer is defined as organization in the brand glossary.
...
- **Wholesale renaming of retired names in prose.** The glossary defines the canonical names and the design's opening maps the old ones, but hundreds of sentences still use tenant, local guard, or protected service. A copy-editing pass should follow once the vocabulary is agreed.
