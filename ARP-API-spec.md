# Oxagen API specification

Proposed interface. These operations describe the contract to build. They are not live endpoints. The proposed paths and typed payloads are specified in the HTTP reference below. They still need implementation and conformance tests.

## Purpose and users

The API gives custom agents, SDKs, company systems, and Oxagen apps one checked way to manage work. It serves the same records and decisions as the [web app](ARP-Web-app-spec.md), [desktop gateway](ARP-Desktop-app-spec.md), [MCP service](ARP-MCP-spec.md), and [CLI](ARP-CLI-spec.md).

A person, agent, or service has its own IAM principal. All three use the same permission layer. Roles grant scoped rights. Record grants, Policies, agent Permissions & limits, and current run state narrow those rights. An API token proves identity; it does not bypass those checks.

## Proposed operations

Use HTTPS and structured JSON for control requests. The overview below groups operations. The HTTP reference later in this spec fixes their proposed paths and schemas; its exact operation IDs take precedence over shorthand in this overview.

| Area | Operations and required behavior |
|---|---|
| Capabilities | `capabilities.negotiate`, `adapter.attest`: agree on protocol version, required controls, and evidence of adapter support. Reject missing required controls. |
| Agents and tools | `agent.resolve`, `toolbelt.resolve`, `toolbelt.status`, `run.mode.change`: resolve the approved agent release and current tools. A mode change needs permission and a fresh access version. |
| Work | `target.list`, `work.submit`, `work.status`, `work.cancel`: choose registered targets, assign work, and track each result. Starting work differs from steering it. |
| Steering and runs | `steering.submit`, `steering.status`, `run.pause`, `run.pause_status`, `run.resume`, `run.stop`: queue changes or request an interruption. Only confirmed boundaries count as paused. |
| Context | `context.resolve`, `memory.query`, `memory.propose`: return allowed context with source versions. A memory proposal cannot grant access. |
...
The caller connects through an enrolled local service. That service resolves the workspace and principal, scans outgoing content, and builds the complete cleaned request. Prompts, attachments, tool results, paths, and metadata follow the same local data rules. An API client may not upload raw material first and ask Oxagen to clean it later.

A proposed `ScanReceipt` binds the enrolled service, workspace, principal/run, policy and scanner versions, complete inspected fields, cleaned artifact references, final request digest, coverage, and expiry. It contains no raw matches or raw digests. The server verifies its issuer and current scope. A client-supplied “scanned” flag is not proof. Unsupported or incomplete inspection blocks the operation.

After cleaning, trusted services check permissions, Policies, record access, and funds. A refund must reserve every applicable limit using authoritative history. Concurrent requests cannot each use the same allowance. A model API proxy alone cannot enforce a refund; the protected tool service must check before it acts.

The final governed action binds the approved cleaned bytes. Changing content requires another scan and decision. Model credentials remain in the proxy. API access cannot create a direct provider route or replace the local guard. See [desktop protection](ARP-design.md#3-desktop-gateway-and-local-data-protection) and [business policies](ARP-design.md#8-tool-belts-permissions-and-business-policies).
...
Proposed version `0.1.0-draft`. These are concrete proposed HTTP paths and payloads, not live endpoints or certification evidence. The machine-readable source is [OpenAPI 3.1 JSON](api/oxagen-openapi-0.1.json). The endpoint inventory is [also available as JSON](api/endpoint-inventory.json).
...
The acting identity of a receipt is never a body field. `source_principal_id` and `connector_deployment_id` are derived by trusted ingress from the mutual-TLS workload identity of the peer that delivers the receipt, and the ingress checks that the named deployment's service principal equals that peer. A body that supplies either value is rejected with 422. Receipts settle holds and financial effects, so a caller-asserted identity would let one connector settle another's actions.

| Field | Type | Required |
|---|---|---|
| `attempt_id` | uuid | yes |
| `source_principal_id` | uuid, server-derived | read only |
| `connector_deployment_id` | uuid or null, server-derived | read only |
...
| `attempt_id` | uuid | yes |
| `audience` | string | yes |
| `requested_seconds` | integer | no |

The server issues `expires_at`; the caller may only ask for a shorter life. The lease is clamped to the least of the request, the credential reference's `max_lease_seconds`, and the reference's own expiry. A caller-chosen expiry was a defect: it let the caller mint a long-lived credential handle. Default ceilings, published with the API: scan receipts 15 minutes, MCP grants 15 minutes, action authorizations 5 minutes, credential leases 15 minutes, gateway certificates 24 hours.
...
This is a complete inventory of the HTTP surface proposed here, not a claim that every provider or platform contract is finished. [Certification blockers](api/CERTIFICATION-BLOCKERS.md) identify what must be resolved and tested. [Validation results](api/validation-report.json) state exactly what was checked. OpenAPI describes HTTP interfaces; it does not enforce policy or prove runtime behavior. [OpenAPI 3.1.1 specification](https://spec.openapis.org/oas/v3.1.1.html)


## HTTP certification gates
