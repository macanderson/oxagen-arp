...
- **Concurrency.** Batches whose dependencies are done run at the same time, each in its own worktree; the pull-request, CI, and merge stages serialize through one lane that rebases and re-reviews a candidate whose base moved. Set `parallel: false` in the run configuration to run one batch at a time, or `maxParallelBatches` to cap a wave. Size the Docker host for the largest wave: each concurrent agent needs its own container, memory, and CPU allotment from `local.execution`.
