...
| `database migrate --no-seed` | Apply reviewed migrations once under a database lock; return nonzero on failure. Grant only needed runtime operations. Create org keys, IAM tables, RLS with `FORCE ROW LEVEL SECURITY`, and tested policies in the real migrations. Do not create a default admin. |
