...
import {assert,digest,hashFile,within,certificationSnapshot,verifyCertificate,validateReview,agentCommand,invokeStage,executionProfileDigest,withoutOperation,activeOperations} from './core.mjs';
...
export class Engine{
 constructor({plan,config,journal,hook}){Object.assign(this,{plan,config,journal,hook});this.repo=path.join(config.controlDir,'product.git');this.local=config.adapter==='local';this.remoteHead=null;this.lane=Promise.resolve();this.reservedCents=0}
 // Every stage that touches the remote repository or moves local main runs through one lane, in order.
 withLane(fn){const run=this.lane.then(fn,fn);this.lane=run.then(()=>{},()=>{});return run}
 save(type,changes){this.journal.record(type,{...this.journal.state,...changes})}
 batchSave(id,value,changes={}){const st=this.journal.state;const stageChanged=value.stage!==st.batches[id]?.stage;const {[id]:_dropped,...rest}=st.pendingReceipts||{};this.save('batch.checkpoint',{batches:{...st.batches,[id]:value},...(stageChanged?{pendingReceipts:rest,...(st.pendingReceipt?.batchId===id?{pendingReceipt:null}:{})}:{}),...changes})}
 pendingFor(id){const st=this.journal.state;return st.pendingReceipts?.[id]??(st.pendingReceipt?.batchId===id?st.pendingReceipt:null)}
 async external(kind,b,payload={}){
  ...
  const pending=this.pendingFor(b.id);if(pending){assert(pending.kind===kind&&pending.payloadDigest===digest(payload),'Pending receipt requires its original transition');return pending.result}
  const cap=kind==='agent'?Math.floor(b.maxCostCents/2):0;
  // Concurrent batches reserve their caps in memory so two of them cannot both pass the run-budget check on the same balance.
  assert((this.journal.state.spentCents||0)+this.reservedCents+cap<=this.config.runBudgetCents,'Run budget would be exceeded');
  ...
  this.reservedCents+=cap;let result;try{result=await invokeStage(this.journal,op,()=>this.hook({...op,timeoutSeconds:Math.min(b.timeoutSeconds,remaining)}))}finally{this.reservedCents-=cap}
  ...
  const receipt={kind,batchId:b.id,payloadDigest:digest(payload),result};const st=withoutOperation(this.journal.state,op.id);
  this.batchSave(b.id,{...record,spentCents:(record.spentCents||0)+result.costCents,lastReceipt:result},{activeOps:st.activeOps,active:st.active,pendingReceipt:receipt,pendingReceipts:{...(this.journal.state.pendingReceipts||{}),[b.id]:receipt},spentCents:(this.journal.state.spentCents||0)+result.costCents});
  return result;
 }
 loadInputs(){
 ...
 // A wave is every batch whose dependencies are done. Batches in a wave run concurrently; the local stages
 // (implement, review, quality) overlap, and the remote stages serialize through the lane. When the lane
 // finds a candidate whose base moved since its review, it rebases the candidate and sends it back for a
 // fresh opposite-harness review before anything is published.
 ready(){const st=this.journal.state.batches;return this.plan.batches.filter(b=>st[b.id]?.stage!=='done'&&b.depends.every(id=>st[id]?.stage==='done'))}
 async run(){
  assert(activeOperations(this.journal.state).length===0&&!this.journal.state.blocked,'Unresolved operation: use status/reconcile');await this.preflight();
  const limit=this.config.parallel===false?1:(this.config.maxParallelBatches??Infinity);
  while(this.plan.batches.some(b=>this.journal.state.batches[b.id]?.stage!=='done')){
   assert(!fs.existsSync(path.join(this.config.controlDir,'CANCEL')),'Cancelled at confirmed between-stage boundary');
   const wave=this.ready().slice(0,limit);assert(wave.length>0,'No batch is ready: a dependency is blocked');
   const outcomes=await Promise.allSettled(wave.map(b=>this.runBatch(b)));
 ...
 rebase(b,r,newBase){
  try{git(['-C',r.worktree,'rebase','--onto',newBase,r.base,r.head])}
  catch(error){try{git(['-C',r.worktree,'rebase','--abort'])}catch{}this.save('rebase.conflict',{blocked:{batchId:b.id,reason:'Base moved and the candidate does not rebase cleanly; operator repair required',knownNoExternalEffect:true}});throw new Error('Base moved and the candidate does not rebase cleanly: '+error.message)}
  const head=git(['-C',r.worktree,'rev-parse','HEAD']);
  if(r.reviewWorktree){try{git(['--git-dir',this.repo,'worktree','remove','--force',r.reviewWorktree])}catch{}}
  this.batchSave(b.id,{...r,head,base:newBase,...(b.phase==='design'?{localBase:newBase}:{}),stage:'review',review:null,reviewWorktree:null,reviewGitDir:null,reviewGitPointerHash:null,receipts:{},rebasedFrom:r.head});
 }
 async runBatch(b){
  for(;;){
   assert(!fs.existsSync(path.join(this.config.controlDir,'CANCEL')),'Cancelled at confirmed between-stage boundary');
   ...
   if(r?.stage==='done'){await this.withLane(()=>r.finished?this.fastForward(r):this.finish(b));r=this.journal.state.batches[b.id];if(r.stage==='done')return;continue}
   for(const id of b.depends)assert(this.journal.state.batches[id]?.stage==='done','Dependency incomplete: '+id);
   if(!r||['retry','preparing'].includes(r.stage)){this.prepare(b);r=this.journal.state.batches[b.id]}
   ...
   if(r.stage==='implement'){
    ...
    await this.external('agent',b,request);r=this.journal.state.batches[b.id];this.verifyBatchCertificate(b,r);const head=this.commit(b,r);this.verifyBatchCertificate(b,r);this.batchSave(b.id,{...r,head,stage:'review'});continue;
   }
   if(r.stage==='review'){
    ...
    this.batchSave(b.id,{...this.journal.state.batches[b.id],review:result.review,stage:'quality'});continue;
   }
   ...
   const i=stages.indexOf(r.stage);assert(i>=0,'Unknown batch stage '+r.stage);const kind=stages[i],next=stages[i+1]||'done';
   if(kind==='quality'){await this.gate(kind,b,next);continue}
   // Remote stages serialize. Before the first publish, a candidate whose base moved is rebased and re-reviewed.
   const rebased=await this.withLane(async()=>{const current=this.journal.state.batches[b.id];if(kind==='pull_request'&&this.local&&current.base!==this.remoteHead){this.rebase(b,current,this.remoteHead);return true}await this.gate(kind,b,next);return false});
   if(rebased)continue;
  }
 }
 // A batch already finished in an earlier run only needs local main caught up.
 fastForward(r){const finalHead=r.mergedHead||r.head,oldBase=r.localBase||r.base;if(this.head()===oldBase)git(['--git-dir',this.repo,'update-ref','refs/heads/main',finalHead,oldBase])}
 // Local main follows the merged result. A design batch that raced another one is rebased onto the moved
 // main and reviewed again before it lands; a product batch's merged head is fetched, so main follows it.
 finish(b){
  const r=this.journal.state.batches[b.id];const finalHead=r.mergedHead||r.head,oldBase=r.localBase||r.base,head=this.head();
  if(b.phase==='design'&&this.local&&head!==oldBase&&head!==finalHead){this.rebase(b,r,head);return}
  if(head!==finalHead){if(head===oldBase)git(['--git-dir',this.repo,'update-ref','refs/heads/main',finalHead,oldBase]);else{git(['--git-dir',this.repo,'merge-base','--is-ancestor',head,finalHead]);git(['--git-dir',this.repo,'update-ref','refs/heads/main',finalHead,head])}}
  if(r.mergedHead)this.remoteHead=r.mergedHead;this.batchSave(b.id,{...this.journal.state.batches[b.id],finished:true});this.save('batch.done',{});
 }
 async gate(kind,b,next){
  let r=this.journal.state.batches[b.id];this.assertUnchanged(r);this.verifyBatchCertificate(b,r);
  ...
  const previous=this.journal.state.batches[b.id];this.batchSave(b.id,{...previous,...(kind==='merge'?{mergedHead:receipt.mergedHead,mergedTree:receipt.mergedTree}:{}),pr:receipt.pr||previous.pr,receipts:{...previous.receipts,[kind]:receipt},stage:next});
 }
}
