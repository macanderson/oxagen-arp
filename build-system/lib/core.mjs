...
export async function processJSON(command,args,input,{timeoutSeconds=60,cancelFile,env={},cwd}={}){
...
function knownCheckFailure(op,result){return ['quality','ci','post_merge_ci'].includes(op.kind)&&result.status==='failed'&&result.operationId===op.id&&result.noExternalEffect===true&&result.allChildrenStopped===true&&result.costCents===0;}
...
export function requestRetry(journal,plan,id){
 ...
 assert(b&&r&&!journal.state.active&&blocked?.knownNoExternalEffect&&blocked.batchId===id&&['review','quality','ci','post_merge_ci'].includes(r.stage),'Retry must target the exact blocked review or check batch');assert(r.attempt<b.maxAttempts,'Retry limit reached');
