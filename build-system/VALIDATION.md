...
The final complete `node --test test/*.test.mjs` run passed **125 tests, 0 failures**. The 22-batch plan passed dry-run validation. A temporary bootstrap installed the AWS modules and templates, left money and release disabled, and produced the correct installation digest. Bootstrap started no agent or external operation.
