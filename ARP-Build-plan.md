...
A partial or unreviewed result still cannot become another batch's base, because a dependency must be `done` before a dependent starts. The controller executes the graph concurrently: every batch whose dependencies are done runs at once, each in its own worktree, and the journal holds every in-flight operation. The implement, review, and local quality stages of one wave overlap. The pull-request, CI, and merge stages serialize through one lane, in arrival order. When the lane takes a candidate whose base moved since its review, it rebases the candidate onto the moved base in place and sends it back for a fresh opposite-harness review before anything is published; a candidate that does not rebase cleanly blocks for operator repair. Design batches, which never leave the machine, go through the same lane and the same rebase-and-re-review when local `main` moved under them. `parallel: false` in the run configuration restores one batch at a time, and `maxParallelBatches` caps a wave. Concurrent batches reserve their per-invocation caps in memory before the run-budget check so two of them cannot both pass on the same balance. `reconcile` clears every in-flight operation, one at a time, by operation id. Product batches may not edit `.github/`, so an implementer cannot rewrite the CI check that gates its own merge; workflow changes need a separate reviewed control run.
